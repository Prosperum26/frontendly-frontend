      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          {[
            ...
            <div
              key={i}
              className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-700 dark:to-slate-700 rounded-2xl p-6 text-center hover:scale-105 transition-transform duration-300"
            >
              <stat.icon className="w-8 h-8 text-blue-600 dark:text-blue-400 mx-auto mb-3" />
              <div className="text-3xl font-black text-heading dark:text-slate-100 mb-1">{stat.value}</div>
              <div className="text-sm font-semibold text-muted dark:text-slate-400">{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-8 mb-16">
          {curriculumData.milestones.map((milestone, index) => (
            <div
              key={milestone.id}
              className="group relative bg-gradient-to-r from-white to-gray-50 dark:from-slate-800 dark:to-slate-700 rounded-2xl p-8 border-2 border-border dark:border-slate-600 hover:border-blue-300 dark:hover:border-blue-500 transition-all duration-300 hover:shadow-xl hover:shadow-blue-500/10"
            >
              {/* Milestone number */}
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                  ...
                  <h3 className="text-2xl font-bold text-heading dark:text-slate-100 mb-2">
                    {milestone.title}
                  </h3>
                  <p className="text-body dark:text-slate-300 mb-4">{milestone.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {milestone.topics.map((topic, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 bg-blue-50 dark:bg-slate-600 text-blue-700 dark:text-blue-300 rounded-full text-xs font-semibold"
                      >
                        {topic}
                      </span>
                    ))}
        ...
        {/* Sample Lessons */}
        <div className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-700 dark:to-slate-800 rounded-3xl p-8 md:p-12">
          <h3 className="text-2xl font-bold text-heading dark:text-slate-100 mb-6 text-center">
            Sample Lessons
          </h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {curriculumData.sampleLessons.map((lesson) => (
              <div
                key={lesson.id}
                className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-border dark:border-slate-600 hover:scale-105 hover:shadow-lg transition-all duration-300 cursor-pointer"
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-muted dark:text-slate-400">{lesson.milestone}</span>
                </div>
                <h4 className="font-bold text-heading dark:text-slate-100 mb-2">{lesson.title}</h4>
                <p className="text-sm text-body dark:text-slate-300 mb-3 line-clamp-2">{lesson.description}</p>
