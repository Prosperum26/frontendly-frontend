...
export function useChatHistory() {
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || (err as Error).message || 'Failed to fetch sessions';
  ...
  const fetchSessionMessages = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      const errorMessage = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || (err as Error).message || 'Failed to fetch messages';
      setError(errorMessage);
      throw err;
    } finally {
      setIsLoading(false);
