    ...
    <div className="min-h-screen flex flex-col bg-main-bg font-sans relative">
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6">
        {isOffline ? (
          ...
          <div className="w-full max-w-lg bg-main-bg rounded-2xl shadow-xl border border-border p-10">
            <div className="text-center mb-8">
              <h1 className="text-2xl font-bold !text-heading">Welcome Back</h1>
              <p className="text-sm text-muted mt-2">Access your technical curriculum and projects.</p>
            </div>

            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className={`block text-xs font-bold uppercase tracking-wide mb-2 ${isError ? 'text-red-500' : 'text-body'}`}>Email Address</label>
                <div className="relative">
                  <div className={`absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none ${isError ? 'text-red-500' : 'text-muted'}`}>
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-1 ${isError ? 'border-red-500 text-red-500 focus:border-red-500 focus:ring-red-500' : 'border-slate-300 focus:border-blue-500 focus:ring-blue-500 bg-surface'}`}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className={`text-xs font-bold uppercase tracking-wide ${isError ? 'text-red-500' : 'text-body'}`}>Password</label>
              ...
              <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 transition-colors mt-2 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div className="mt-8 flex items-center justify-between">
              ...
              <span className="px-3 text-xs text-muted font-semibold uppercase tracking-wider whitespace-nowrap">Or continue with</span>
              <hr className="w-full border-border" />
            </div>
      ...
      <footer className="bg-surface border-t border-border py-12 px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between">
          <div className="mb-8 md:mb-0 max-w-sm">
            ...
            <p className="text-sm font-semibold text-muted mb-4">Keep going, you're doing great!</p>
